...
def execute_command(the_command, m):
    ...
    the_functions = dict(captureImages=m.cmd_captureImages,brightStarCoords=m.cmd_brightStarCoords,adjustExposure=m.cmd_adjustExposure,setCameraValues=m.cmd_setCameraValues,starDistanceFromCenter=m.cmd_starDistanceFromCenter,orientationCapture=m.cmd_orientationCapture,focusCapture=m.cmd_focusCapture,calculateCameraOrientation=m.cmd_calculateCameraOrientation,calibrateMagnitude=m.cmd_calibrateMagnitude,Chop=m.cmd_Chop,imageCube=m.cmd_imageCube,defineCenter=m.cmd_defineCenter,centerIsHere=m.cmd_centerIsHere)
