...
def execute_command(the_command, m):
    ...
    the_functions = dict(checkTemperature=m.cmd_checkTemperature,setTemperature=m.cmd_setTemperature,establishLink=m.cmd_establishLink,closeLink=m.cmd_closeLink,getCCDParams=m.cmd_getCCDParams,enableRegulation=m.cmd_enableRegulation,disableRegulation=m.cmd_disableRegulation,changeBinning=m.cmd_changeBinning,imageType=m.cmd_imageType,exp=m.cmd_exp,exposeAndWait=m.cmd_exposeAndWait,abortExposure=m.cmd_abortExposure,ljtemp=m.cmd_ljtemp,heater=m.cmd_heater,backLED=m.cmd_backLED,ippower=m.cmd_ippower,agitator=m.cmd_agitator,status=m.cmd_status,inject=m.cmd_inject,abortLoop=m.cmd_abortLoop,pulse=m.cmd_pulse)
