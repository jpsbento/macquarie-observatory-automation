...
class BisqueMountServer:
	...
	def cmd_look(self,the_command):
		...
		if len(commands) == 2:
			if commands[1] == 'north' or commands[1] == 'North':
				...
				command = '/* Java Script */TheSkyXAction.execute(TheSkyXAction.LookDown); var Out; Out = "OK"'
				client_socket.send(command)
				return self.messages()
	...
	def cmd_find(self,the_command):
		...
		if len(commands) == 2:
			obj = commands[1]
			command = '/* Java Script */var Out; var PropCnt = 189; Out = "";sky6StarChart.Find("'+obj+'"); for(p=0;p<PropCnt;++p) {if(sky6ObjectInformation.PropertyApplies(p) != 0){sky6ObjectInformation.Property(p) ;Out+=sky6ObjectInformation.ObjInfoPropOut}}'
			client_socket.send(command)
			return self.messages()
