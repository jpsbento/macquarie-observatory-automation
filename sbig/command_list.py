...
def execute_command(the_command, m):
    ...
    the_functions = dict(abortExposure=m.cmd_abortExposure,closeLink=m.cmd_closeLink,establishLink=m.cmd_establishLink,getCCDParams=m.cmd_getCCDParams,setTemperature=m.cmd_setTemperature,checkTemperature=m.cmd_checkTemperature,disableRegulation=m.cmd_disableRegulation,exposeAndWait=m.cmd_exposeAndWait,focusCalculate=m.cmd_focusCalculate,focusImages=m.cmd_focusImages,imageInstruction=m.cmd_imageInstruction,imagingStatus=m.cmd_imagingStatus)
    commands = string.split(the_command)
    if len(commands) == 0:
        return ""
    if commands[0] == "help":
        if (len(commands) == 1):
            return 'abortExposure\ncloseLink\nestablishLink\ngetCCDParams\nsetTemperature\ncheckTemperature\ndisableRegulation\nexposeAndWait\nfocusCalculate\nfocusImages\nimageInstruction\nimagingStatus'
