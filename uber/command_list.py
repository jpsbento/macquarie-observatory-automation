...
def execute_command(the_command, m):
    ...
    the_functions = dict(finishSession=m.cmd_finishSession,startSession=m.cmd_startSession,labjack=m.cmd_labjack,telescope=m.cmd_telescope,weatherstation=m.cmd_weatherstation,camera=m.cmd_camera,sidecam=m.cmd_sidecam,fiberfeed=m.cmd_fiberfeed,labjacku6=m.cmd_labjacku6,setDomeTracking=m.cmd_setDomeTracking,orientateCamera=m.cmd_orientateCamera,offset=m.cmd_offset,focusStar=m.cmd_focusStar,focusIRAF=m.cmd_focusIRAF,override_wx=m.cmd_override_wx,guiding=m.cmd_guiding,spiral=m.cmd_spiral,centerStar=m.cmd_centerStar,masterAlign=m.cmd_masterAlign,Imaging=m.cmd_Imaging,Imsettings=m.cmd_Imsettings,checkIfReady=m.cmd_checkIfReady)
